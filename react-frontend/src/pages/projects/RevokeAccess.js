...
const RevokeAccess = () => {
    ...
    useEffect(() => {
        const fetchAllUsers = async () => {
            try {
                const res = await axios.get(`${config.API_URL}:${config.API_PORT}/Users/fetchRevoke/${projectId}`);
                setUsers(res.data);
                console.log(res);
            } catch (err) {
                console.log(err);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await axios.post(`${config.API_URL}:${config.API_PORT}/RevokeUserAccess`, { projectId, projectName, user });
            if (res.data.message === 'Successful') {
                console.log('Successfully Updated');
                history.push('/app/projects'); // Redirect to projects page
            } else {
                setError('Error in revoking user access.');
            }
        } catch (err) {
            console.log(err);
            setError('Error in revoking user access.');
